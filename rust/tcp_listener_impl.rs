...
pub extern "C" fn tcp_accept_cb(arg: *mut raw::c_void, newpcb: *mut tcp_pcb, err: err_t) -> err_t {
    ...
    let stream = TcpStreamImpl::new(newpcb, listener.shard);
...
impl TcpListenerImpl {
    pub fn new(shard: ShardRef) -> Result<Box<Self>, Error> {
        unsafe {
            let _g = shard.mutex.lock();
            let mut tpcb = (shard.vt.tcp_new)();
            let err = (shard.vt.tcp_bind)(tpcb, &ip_addr_any_type, 0);
            if err != err_enum_t_ERR_OK as err_t {
                error!("bind TCP failed: {}", err);
                return Err(Error::LwIP(err));
            }
            let mut reason: err_t = 0;
            tpcb = (shard.vt.tcp_listen_with_backlog_and_err)(
                tpcb,
            ...
            (shard.vt.tcp_arg)(tpcb, arg);
            (shard.vt.tcp_accept)(tpcb, Some(tcp_accept_cb));
            Ok(listener)
        }
    }
}
...
impl Drop for TcpListenerImpl {
    fn drop(&mut self) {
        unsafe {
            ...
            (self.shard.vt.tcp_arg)(self.tpcb as *mut tcp_pcb, null_mut());
            (self.shard.vt.tcp_accept)(self.tpcb as *mut tcp_pcb, None);
            (self.shard.vt.tcp_close)(self.tpcb as *mut tcp_pcb);
        }
    }
}
